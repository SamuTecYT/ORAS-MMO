...
1. Copia el archivo **`plugin.3gx`** (o `OrasMmo.3gx`).
...
1. **Instalar ZeroTier:**  
   En la carpeta `ZeroTier/` (o en `Para_PC/`), haz doble clic en **`ZeroTier One.msi`** para instalarlo.
...
- El anfitrión abre `Servidor/run_server.bat` y mira su IP local (ejemplo: `192.168.1.15`). Esa será la IP a usar.

---

## 🚀 Paso 4: Iniciar el Servidor (Solo el Anfitrión)
Si tú eres el anfitrión del grupo:
1. Entra a la carpeta **`Servidor/`** y haz doble clic en **`run_server.bat`**.
